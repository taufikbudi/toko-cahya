...
mysql_select_db($database_toko_cahya, $toko_cahya);
$query_Recordset1 =  sprintf("SELECT sum( total_bayar) FROM user_order WHERE user_order.userName = %s" , GetSQLValueString($colname_tampil_user, "text"));
...
mysql_select_db($database_toko_cahya, $toko_cahya);
$query_keranjang = sprintf("SELECT * FROM user,user_order,list_barang WHERE user.userName=user_order.userName AND user_order.id_barang=list_barang.id_barang AND user.userName = %s AND status_order=0", GetSQLValueString($colname_keranjang, "text"));
...
<li class="harga"> Keranjang Belanja <?php echo $row_tampil_user['nama_lengkap']; ?> </li>
<?php 
class Status { 
...
<td>
<span style="float:right"><form action="hapus.php?id_order=<?php echo $row_keranjang['id_order']; ?>" method="post">
Jumlah order : <input size="1" class="jumlah" name="jumlah_order" value="<?php echo $row_keranjang['jumlah_order']; ?>" readonly type="text">Unit <input name="" type="submit" value="Hapus" class="hapus">
<input type="hidden" name="id_barang" value="<?php echo $row_keranjang['id_barang']; ?>"> 
<input type="hidden" name="stok" value="<?php echo $row_keranjang['stok']; ?>"> 
</form></span>
</td>
</tr>
</table>
...
<li class="beli"><a href="bayar.php"> Bayar </a></li> 
<?php } 
else {
